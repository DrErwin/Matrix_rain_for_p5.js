  ...
  peer.on('connection', function (conn) {
    ...
    connection.on('data', function (dataReceived) {
      ...
      switch (dataReceived.event) {
        ...
        case 'feed':
          chooseCamera(null, dataReceived.data.feed);
          console.log('Feed received: '+dataReceived.data.feed);
          break;
  ...
  if (kinect.constructor.name === 'KinectAzure') {
    ...
    if (kinect.open()) {
      kinect.on('multiSourceFrame', function (frame) {
        ...
        let bodyList = []
        let isDetect = false
        for(let i = 0; i < frame.body.bodies.length; i++) {
          bodyList.push(i)
        }
        kinect.trackPixelsForBodyIndices(bodyList);
        KeyData = new Array(imageDataSize).fill(0);
        
        for(var bodyNum = 0; bodyNum<frame.body.bodies.length; bodyNum++){
          if(frame.body.bodies[bodyNum].tracked && frame.bodyIndexColor.bodies[bodyNum].buffer){
            isDetect = true
            newPixelData = frame.bodyIndexColor.bodies[bodyNum].buffer;
            for (let i = 0; i < imageDataSize; i++) {
              imageDataArray[i] = newPixelData[i];
              KeyData[i] += imageDataArray[i];
            }
          }
        }
